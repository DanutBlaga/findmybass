...
class SQLUtils
{
     ...
     * @return string
     */
    protected static function makeLikeParam($search, $pattern = '%%%s%%')
    {
        /**
    ...
    public static function normalizeArrayElementsAndImplode(array $array, $implodeChar) {

        foreach ($array as &$word) {
            $word = StringNormalizer::normalizeName($word);
            $word = self::makeLikeParam($word, "%s");
        }

        return $implodeChar.implode($implodeChar, $array).$implodeChar;
    }
